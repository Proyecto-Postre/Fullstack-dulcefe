...
## 🎯 Propósito
Recupera el detalle completo de una orden específica por su identificador UUID, incluyendo la relación con `profiles`, `order_items` y `products`.

## 🔐 Requisitos de Seguridad
* **Auth:** 🔒 `requireAdmin(event)` (JWT de Supabase con `is_admin: true` en `profiles`).
* **Cabeceras:** `Authorization: Bearer <TOKEN>`.
* **Ejecución:** Backend seguro con Service Role en el servidor Nuxt.
...
* `400 Bad Request` (`VALIDATION_ERROR`): ID de orden no especificado.
