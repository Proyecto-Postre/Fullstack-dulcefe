  ...
  <div class="flex-1 w-full bg-[#F4F1E1]/40 pt-2.5 pb-4 sm:pt-4 sm:pb-6 px-3 sm:px-6 lg:px-8 flex flex-col justify-start">
    <div class="max-w-5xl mx-auto w-full space-y-3 sm:space-y-3.5">
      ...
      <div class="flex items-center justify-between gap-2">
        ...
        >
          <Icon :name="isCopied ? 'lucide:check' : 'lucide:share-2'" class="w-3.5 h-3.5 transition-transform duration-200" :class="isCopied ? 'text-emerald-700 scale-110' : 'text-[#4A5D23]'" />
      ...
      <div v-else class="space-y-3 sm:space-y-3.5">
        ...
        <div class="bg-gradient-to-br from-[#F4F1E1] via-white to-[#F4F1E1]/80 rounded-2xl sm:rounded-3xl p-3 sm:p-3.5 border border-[#4A5D23]/20 shadow-soft-sm flex flex-col sm:flex-row items-center justify-between gap-2.5">
          <div class="space-y-0.5 text-center sm:text-left min-w-0">
            <div class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-100/90 text-emerald-900 text-[10px] font-bold border border-emerald-300 shadow-2xs">
              <Icon name="lucide:message-circle" class="w-3 h-3 text-emerald-700" />
