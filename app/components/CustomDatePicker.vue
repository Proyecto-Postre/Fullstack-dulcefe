  ...
  <div class="relative w-full" ref="datePickerContainer">
    <div class="relative">
      <input 
        ...
        maxlength="10"
        :class="[
          'w-full border border-[#4A5D23]/20 rounded-xl px-3 py-2.5 text-[#2A321B] font-bold focus:outline-none focus:ring-2 focus:ring-[#4A5D23]/10 focus:border-[#4A5D23] transition-all placeholder:text-[#4A5D23]/40 shadow-sm text-sm',
          bgClass || 'bg-white'
        ]"
      >
      <button 
        @click.prevent="toggleDatePicker"
        class="absolute right-3 top-1/2 -translate-y-1/2 text-[#2A321B] hover:text-[#4A5D23] hover:bg-[#F4F1E1] p-1.5 rounded-lg transition-colors"
      >
      ...
      <Transition name="pop">
        <div 
          ...
          :class="[
            'absolute z-50 left-0 w-64 bg-white border border-[#4A5D23]/20 rounded-xl shadow-xl p-4',
            datePickerPosition === 'top' ? 'bottom-full mb-2' : 'top-full mt-2'
          ]"
        >
          <div class="flex items-center justify-between mb-4">
            <button @click.prevent="prevMonth" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#F4F1E1] border border-transparent hover:border-[#2A321B]/20 transition-all text-[#4A5D23]">
              <Icon name="lucide:chevron-left" class="w-4 h-4" />
            </button>
            <span class="font-bold text-[#2A321B]">{{ monthNames[currentMonth] }} {{ currentYear }}</span>
            <button @click.prevent="nextMonth" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#F4F1E1] border border-transparent hover:border-[#2A321B]/20 transition-all text-[#4A5D23]">
          ...
          <div class="grid grid-cols-7 gap-1">
            ...
            <button 
              ...
              :class="[
                ...
                isPastDate(day)
                  ? 'text-[#2A321B]/30 cursor-not-allowed'
                  : 'text-[#2A321B] hover:bg-[#4A5D23] hover:text-white cursor-pointer'
              ]"
            >
              {{ day }}
            </button>
